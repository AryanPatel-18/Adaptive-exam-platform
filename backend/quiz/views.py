...
from quiz.selectors import AttemptSelector, QuizStatsSelector
...
class StartQuizAPIView(APIView):
    ...
    def post(self, request, quiz_id):
        ...
        ActivityLogger.log(
            user=request.user,
            action="QUIZ_STARTED",
            description=f"Started an attempt for quiz '{attempt.quiz.title}'.",
            metadata={"quiz_id": str(quiz_id), "attempt_id": str(attempt.id)}
        )
...
class WorkspaceQuizStatsAPIView(APIView):
    ...
    def get(self, request, workspace_id):
        stats_data = QuizStatsSelector.get_workspace_quiz_stats(
            workspace_id=workspace_id,
            user=request.user,
        )
        serializer = WorkspaceQuizStatsSerializer(stats_data)
